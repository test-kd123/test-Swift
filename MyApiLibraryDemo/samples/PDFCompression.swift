...
class PDFCompression: NSObject {
    ...
    class func entrance() {
        // Create a task
        self.client.createTask(url: CPDFDocumentEditor.COMPRESS) { taskId, param in
...
//            let path = Bundle.main.path(forResource: "test", ofType: "pdf")
            ...
            self.client.uploadFile(filepath: path!, password: "1234", params: [CPDFFileUploadParameterKey.quality.string():"50"], taskId: _taskId) { filekey, fileUrl, _ in
                group.leave()
            }
...
//                                }
    ...
    class func asyncEntrance() {
        Task { @MainActor in
            // Create a task
            let taskId = await self.client.createTask(url: CPDFDocumentEditor.COMPRESS) ?? ""

            // upload File
            let path = Bundle.main.path(forResource: "test", ofType: "pdf")
            let (fileKey, fileUrl, error) = await self.client.uploadFile(filepath: path ?? "", password: "", params: [CPDFFileUploadParameterKey.quality.string():"50"], taskId: taskId)
