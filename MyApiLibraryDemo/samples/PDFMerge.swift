...
class PDFMerge: NSObject {
    ...
    class func entrance() {
        self.client.createTask(url: CPDFDocumentEditor.MERGE) { taskId, param in
...
//            let path = Bundle.main.path(forResource: "test", ofType: "pdf")
            ...
            self.client.uploadFile(filepath: path!, password: "1234", params: [CPDFFileUploadParameterKey.pageOptions.string():["1,2"]], taskId: _taskId) { filekey, fileUrl, _  in
                group.leave()
            }
            group.enter()
            self.client.uploadFile(filepath: path!, password: "1234", params: [CPDFFileUploadParameterKey.pageOptions.string():["1,2"]], taskId: _taskId) { filekey, fileUrl, _  in
...
//                                }
    ...
    class func asyncEntrance() {
        Task { @MainActor in
            //Create a task
            let taskId = await self.client.createTask(url: CPDFDocumentEditor.MERGE) ?? ""

            // upload File
            let path = Bundle.main.path(forResource: "test", ofType: "pdf")
            let (fileKey, fileUrl, error) = await self.client.uploadFile(filepath: path ?? "", password: "", params: [CPDFFileUploadParameterKey.pageOptions.string():["1,2"]], taskId: taskId)
            let (fileKey2, fileUrl2, error2) = await self.client.uploadFile(filepath: path ?? "", password: "", params: [CPDFFileUploadParameterKey.pageOptions.string():["1,2"]], taskId: taskId)
