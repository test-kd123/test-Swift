...
//                                }
    ...
    class func asyncEntrance() {
        Task { @MainActor in
            let taskId = await self.client.createTask(url: CPDFDocumentEditor.DELETE)
//
            let path = Bundle.main.path(forResource: "test", ofType: "pdf")
            let _ = await self.client.uploadFile(filepath: path ?? "", params: [CPDFFileUploadParameterKey.pageOptions.string():["1"]], taskId: taskId ?? "")
