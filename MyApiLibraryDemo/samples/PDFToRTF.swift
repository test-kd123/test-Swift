...
class PDFToRTF: NSObject {
    ...
    class func entrance() {
        // Create a task
        self.client.createTask(url: CPDFConversion.PDF_TO_RTF) { model in
...
//            let path = Bundle.main.path(forResource: "test", ofType: "pdf")
            ...
            self.client.uploadFile(filepath: path!, password: "1234", params: [CPDFFileUploadParameterKey.isContainAnnot.string() : "1", CPDFFileUploadParameterKey.isContainImg.string() : "1"], taskId: _taskId) { uploadFileModel in
                group.leave()
            }
    ...
    class func asyncEntrance() {
        Task { @MainActor in
            // Create a task
            let taskModel = await self.client.createTask(url: CPDFConversion.PDF_TO_RTF)
