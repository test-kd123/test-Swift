...
class DeleteWatermark: NSObject {
    ...
    class func entrance() {
        // Create a task
        self.client.createTask(url: CPDFDocumentEditor.DEL_WATERMARK) { model in
...
//            let path = Bundle.main.path(forResource: "test", ofType: "pdf")
            ...
            self.client.uploadFile(filepath: path!, password: "1234", params: [:], taskId: _taskId) { uploadFileModel in
                group.leave()
            }
    ...
    class func asyncEntrance() {
        Task { @MainActor in
            // Create a task
            let taskModel = await self.client.createTask(url: CPDFDocumentEditor.DEL_WATERMARK)
            let taskId = taskModel?.taskId ?? ""

            // upload File
            let path = Bundle.main.path(forResource: "test", ofType: "pdf")
            let uploadFileModel = await self.client.uploadFile(filepath: path ?? "", password: "" ,params: [:], taskId: taskId)
