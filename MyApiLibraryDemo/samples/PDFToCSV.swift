...
class PDFToCSV: NSObject {
    ...
    class func entrance() {
        self.client.createTask(url: CPDFConversion.PDF_TO_CSV) { taskId, param in
...
//            let path = Bundle.main.path(forResource: "test", ofType: "pdf")
            ...
            self.client.uploadFile(filepath: path!, password: "1234", params: [CPDFFileUploadParameterKey.isCsvMerge.string() : "1"], taskId: _taskId) { filekey, fileUrl, _ in
                group.leave()
            }
...
//                                }
    ...
    class func asyncEntrance() {
        Task { @MainActor in
            // Create a task
            let taskId = await self.client.createTask(url: CPDFConversion.PDF_TO_CSV) ?? ""

            // upload File
            let path = Bundle.main.path(forResource: "test", ofType: "pdf")
            let (fileKey, fileUrl, error) = await self.client.uploadFile(filepath: path ?? "",  password: "",params: [CPDFFileUploadParameterKey.isCsvMerge.string() : "1"], taskId: taskId)
