...
class OCR: NSObject {
    ...
    class func entrance() {
        // Create a task
        self.client.createTask(url: CPDFDocumentAI.OCR) { model in
            ...
            self.client.uploadFile(filepath: path!, params: [CPDFFileUploadParameterKey.lang.string():"auto"], taskId: _taskId) { uploadFileModel in
                group.leave()
            }
    ...
    class func asyncEntrance() {
        Task { @MainActor in
            // Create a task
            let taskModel = await self.client.createTask(url: CPDFDocumentAI.OCR)
            let taskId = taskModel?.taskId ?? ""

            // upload File
            let path = Bundle.main.path(forResource: "test", ofType: "png")
            let uploadFileModel = await self.client.uploadFile(filepath: path ?? "", params: [
